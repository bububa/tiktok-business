...
// AutoMessageGetRequest Get the automatic messages for a Business Account API Request
type AutoMessageGetRequest struct {
...
func (r *AutoMessageGetRequest) Encode() string {
	values := util.NewURLValues()
	values.Set("business_id", r.BusinessID)
	values.Set("auto_message_type", string(r.AutoMessageType))
	if r.AutoMessageID != "" {
		values.Set("auto_message_id", r.AutoMessageID)
	}
	ret := values.Encode()
...
// AutoMessageGetResponse Get the automatic messages for a Business Account API Response
type AutoMessageGetResponse struct {
	model.BaseResponse
	Data *AutoMessageGetResult `json:"data,omitempty"`
}

type AutoMessageGetResult struct {
	// BusinessID Application specific unique identifier for the TikTok Business Account
	BusinessID string `json:"business_id,omitempty"`
	// OperationStatus The status of the automatic message.
	// Enum values:
	// ENABLE: The automatic message is turned on.
	// DISABLE: The automatic message is turned off.
	// If you want to turn on or turn off an automatic message, use /business/message/auto_message/status/update/.
	OperationStatus enum.OperationStatus `json:"operation_status,omitempty"`
	// AutoMessages The list of automatic messages.
	AutoMessages []AutoMessage `json:"auto_messages,omitempty"`
}
